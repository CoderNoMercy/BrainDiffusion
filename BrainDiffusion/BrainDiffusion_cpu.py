...
import numpy as np
import pandas as pd
from BrainDiffusion.operators_cpu import *
...
class params:
    ...
    '''
    def __init__(self, path, dat_dir, diff_time):
        ...
        self.labels = np.asarray(nib.load(path['labels']).get_fdata()) # template file
        ...
        N = 256
        self.N = N
        self.sigma = 2 * np.pi / N

        dx = 2 * np.pi / N
        tmp = 1j * np.fft.fftfreq(N, dx)

        assert (max(tmp.shape) == N)

        self.IOmega_x, self.IOmega_y, self.IOmega_z = np.meshgrid(tmp, tmp, tmp)

        assert (self.IOmega_x.shape == (N, N, N))
        assert (self.IOmega_y.shape == (N, N, N))
        assert (self.IOmega_z.shape == (N, N, N))

    def forward(self, roi_id, subj_name):
    ...
    def add_seg_diff_tensor(self):
        ...
        self.wm_ref = np.array(tmp.copy())
        self.gm_ref = np.array(tmp2.copy())

        ## apply gaussian filter to smooth the boundary of regions
        self.wm = gaussian_filter(np.array(tmp.copy()), 1)
        self.gm = gaussian_filter(np.array(tmp2.copy()), 1)
        csf = gaussian_filter(np.array(tmp3.copy()), 1)
        vt = gaussian_filter(np.array(tmp4.copy()), 1)
        bg = gaussian_filter(np.array(tmp5.copy()), 1)
        ...
        L1 = np.array(nib.load(self.path['L1']).get_fdata())
        L2 = np.array(nib.load(self.path['L2']).get_fdata())
        L3 = np.array(nib.load(self.path['L3']).get_fdata())
        V1 = np.array(nib.load(self.path['V1']).get_fdata())
        V2 = np.array(nib.load(self.path['V2']).get_fdata())
        V3 = np.array(nib.load(self.path['V3']).get_fdata())

        V1_T = np.expand_dims(V1, axis=3)
        V2_T = np.expand_dims(V2, axis=3)
        V3_T = np.expand_dims(V3, axis=3)

        V1 = np.expand_dims(V1, axis=4)
        V2 = np.expand_dims(V2, axis=4)
        V3 = np.expand_dims(V3, axis=4)

        L1 = np.expand_dims(L1, axis=3)
        L1 = np.expand_dims(L1, axis=4)
        L2 = np.expand_dims(L2, axis=3)
        L2 = np.expand_dims(L2, axis=4)
        L3 = np.expand_dims(L3, axis=3)
        L3 = np.expand_dims(L3, axis=4)

        L1 = np.tile(L1, (1, 1, 1, 3, 3))
        L2 = np.tile(L2, (1, 1, 1, 3, 3))
        L3 = np.tile(L3, (1, 1, 1, 3, 3))
...
def eval_diffusion(use_default_data,
    ...
    test_params = params(path, dat_dir, diff_time)
    ...
    test_params.add_seg_diff_tensor()

    if not os.path.exists(test_params.out_dir):
        os.mkdir(test_params.out_dir)

    for (i_roi, roi) in enumerate(labels_list):

        fname_1 = os.path.join(test_params.out_dir, 'c1_%d.nii.gz' % roi)
        if os.path.exists(fname_1):
            continue

        print('Now executing ROI: ', roi, ' ---progress: {}/{}'.format(i_roi+1, len(labels_list)))
        result = test_params.forward(roi, subj_name)
        writeNII(result[1], fname_1, ref_image=test_params.affine)
